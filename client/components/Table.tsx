...
const stripComponents = (components: Record<string, (props: any) => React.ReactNode>) =>
...
const Table: React.FC<BaseProps> & {
	...
	const containsTable = childrenArray.some(
		child =>
			...
			((typeof child.type === 'string' && child.type === 'table') || (child.type as any)?.displayName === 'Table'),
	);
	...
	childrenArray.forEach((child, idx) => {
		...
		if (React.isValidElement(child) && (child.type === Row || (child.type as any)?.displayName === 'Row') && idx < childrenArray.length - 1) {
			withSeparators.push(<Separator key={`sep-${idx}`} />);
		}
	});

	return containsTable ? children : (
			<table className={cn('w-full grid grid-cols-1 auto-rows-fr gap-1.5 grid-flow-row', className)} {...props}>
				{withSeparators}
			</table>
		);
};
...
const Row: React.FC<BaseProps> = ({ children, className = '', ...props }) => {
	...
	const normalizeChildren = (node: React.ReactNode): React.ReactNode[] => {
		...
		if (React.isValidElement(node) && node.type === React.Fragment) return normalizeChildren((node.props as any)?.children);
		return [node];
	};

	const isCellNode = (node: React.ReactNode) =>
		React.isValidElement(node) && (typeof node.type === 'string' ? node.type === 'td' : (node.type as any)?.displayName === 'Col');
	...
	const splitMixedNodesIntoCellGroups = (nodes: React.ReactNode[]) => {
		...
		nodes.forEach(node => {
			...
			if (React.isValidElement(node) && typeof node.type === 'string' && node.type === 'br') {
				pushSeparator();
				return;
			}
	...
	if (items.length === 1 && React.isValidElement(items[0]) && !isCellNode(items[0])) {
		const unwrapped = normalizeChildren((items[0].props as any)?.children);
		if (unwrapped.length > 0) items = unwrapped;
	}

	let content: React.ReactNode;
	const makeCols = (values: React.ReactNode[]) => values.map((value, index) => <Col key={index}>{value}</Col>);
	const primitiveItems = items.filter(item => typeof item === 'string' || typeof item === 'number') as Array<string | number>;
	const onlyPrimitives = primitiveItems.length === items.length;

	if (onlyPrimitives && primitiveItems.length === 1 && typeof primitiveItems[0] === 'string') {
	...
	if (content === undefined) {
		if (onlyPrimitives) {
			...
			if (grouped.length > 1) {
				content = makeCols(grouped.map(group => (group.length === 1 ? group[0] : <>{group}</>)));
			} else {
	...
	return (
		<tr className={cn('grid grid-rows-1 auto-cols-fr grid-flow-col justify-center size-fit m-auto', className)} {...props}>
			{content}
		</tr>
	);
};
...
const Col: React.FC<BaseProps> = ({ children, className = '', ...props }) => {
	const renderCellContent = () => {
		...
		if (typeof children === 'string') {
			return (
				<ReactMarkdown remarkPlugins={[remarkDirective, remarkElements, remarkGfm]} components={tableCellMarkdownComponents}>
					{children}
				</ReactMarkdown>
			);
		}
	...
	return (
		<td className={cn('col-span-1 items-center size-fit m-auto', className)} {...props}>
			{renderCellContent()}
		</td>
	);
};

const Separator: React.FC<SeparatorProps> = ({ span = 1000, className = '', ...props }) => (
